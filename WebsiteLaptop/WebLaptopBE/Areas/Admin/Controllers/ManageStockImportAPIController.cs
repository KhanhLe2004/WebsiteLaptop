...
namespace WebLaptopBE.Areas.Admin.Controllers
{
    ...
    public class ManageStockImportAPIController : ControllerBase
    {
        ...
        public ManageStockImportAPIController(WebLaptopTenTechContext context, HistoryService historyService)
        {
            _context = context;
            ...
            [FromQuery] DateTime? dateTo = null)
        {
            try
            {
                ...
                {
                    query = query.Where(si => si.Time.HasValue && si.Time.Value.Date >= dateFrom.Value.Date);
                }
                if (dateTo.HasValue)
                {
                    query = query.Where(si => si.Time.HasValue && si.Time.Value.Date <= dateTo.Value.Date);
                }
